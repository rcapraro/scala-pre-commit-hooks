import argparse
from pre_commit_hooks.sbt_runner import *
...
def main(argv=None):
    arg_p = argparse.ArgumentParser(description='Run SBT compilation with fatal warnings')
    arg_p.add_argument(f'--{ARG_ADDITIONAL_ARGS}',
                       action='append',
                       help='Additional arguments for scalac, such as warning flags, can be multi-valued.')

    arg_p.add_argument(f'--{ARG_COMPILE_SCOPE}',
                       default=DEFAULT_COMPILE_SCOPE,
                       help=f'Compile scope for the check. Default: {DEFAULT_COMPILE_SCOPE}')

    args = vars(arg_p.parse_args(argv))

    if args[ARG_ADDITIONAL_ARGS] is not None:
        args[ARG_ADDITIONAL_ARGS].append("-Xfatal-warnings")
    else:
        args[ARG_ADDITIONAL_ARGS] = "-Xfatal-warnings"
